...
bool ZipFileRead::ExtractFileTo(const std::string& filename, vfs::Path destPath)
{
	...
	vfs::Path remFile = destPath;
	vfs::NFile::remove( remFile );
	...
	if (outFile == NULL) 
	{
		...
		vfs::Directory directory = destPath.getDir();

		if( !directory.isExist() )
		{
			// Directory isn't there. Try to create it.

			if( vfs::Directory::create( directory.toString() ) )
			{
...
std::list<vfs::Path> ZipFileRead::ExtractAllFilesTo(vfs::Directory destdir,
												   ...
												   const std::set<std::string>& ignoreList)
{
	...
	{
		vfs::Path destFile = destdir.getFilePath( filesToExtract[i] );
...
bool ZipFileWrite::DeflateFile(vfs::Path fileToCompress, const std::string& destPath, CompressionMethod method)
{
...
ZipFileReadPtr Zip::OpenFileRead(vfs::Path fullPath)
{
...
ZipFileWritePtr Zip::OpenFileWrite(vfs::Path fullPath, WriteMode mode)
{
...
void Zip::RemoveFilesFromArchive(vfs::Path fullPath, const std::set<std::string>& membersToRemove)
{
