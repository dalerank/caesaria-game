...
public:
	...
	typedef SmartPtr<CompressedFile> CompressedFilePtr;
...
public:
	 ...
	 */
	bool ExtractFileTo(const std::string& filename, vfs::Path destPath);
	 ...
	 */
	std::list<vfs::Path> ExtractAllFilesTo(vfs::Directory destPath,
	 ...
	 */
	unsigned int GetCumulativeCrc();
};
typedef SmartPtr<ZipFileRead> ZipFileReadPtr;

/**
 * A class wrapping around a minizip file handle, for storing data
 * into ZIP files.
 */
class ZipFileWrite : public ReferenceCounted
{
public:
...
public:
	 ...
	 * @returns: TRUE on success, FALSE otherwise.
	 */
	bool DeflateFile(vfs::Path fileToCompress, const std::string& destPath, CompressionMethod method = DEFLATE);
...
typedef SmartPtr<ZipFileWrite> ZipFileWritePtr;
...
public:
	 ...
	 */
	static ZipFileReadPtr OpenFileRead( vfs::Path fullPath);
	 ...
	 * @returns: NULL on failure, the file object on success.
	 */
	static ZipFileWritePtr OpenFileWrite(vfs::Path fullPath, WriteMode mode);
	 ...
	 */
	static void RemoveFilesFromArchive(vfs::Path fullPath, const std::set<std::string>& membersToRemove);
};
