...
public:
	 ...
	 */
	bool ExtractFileTo(const std::string& filename, const io::FilePath& destPath);
...
public:
	 ...
	 * @returns: TRUE on success, FALSE otherwise.
	 */
	bool DeflateFile(const io::FilePath& fileToCompress, const std::string& destPath, CompressionMethod method = DEFLATE);
...
public:
	 ...
	 */
	static ZipFileReadPtr OpenFileRead(const io::FilePath& fullPath);
	 ...
	 * @returns: NULL on failure, the file object on success.
	 */
	static ZipFileWritePtr OpenFileWrite(const io::FilePath& fullPath, WriteMode mode);
	 ...
	 */
	static void RemoveFilesFromArchive(const io::FilePath& fullPath, const std::set<std::string>& membersToRemove);
};
