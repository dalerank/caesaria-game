...
game.ui.infobox.baseSpecialOrdersWindow = function(top, left, height) {
  ...
  ibox.gbOrders.addProduct = function(gtype, storage) {
    ...
    lb.textAlign = { h:"upperLeft", v:"center" };

    var btnOrder = ibox.gbOrders.addButton(140, ry, 240, 25);
    btnOrder.style = "blackBorderUp";
    btnOrder.font = "FONT_1_WHITE";
    btnOrder.callback = function() { ibox.gbOrders.changeRule(btnOrder, gtype, storage); }

    var img = new Image(ibox.gbOrders);
    img.geometry = {x:ibox.gbOrders.w-35, y:ry, w:25, h:25};
    ...
    btnVolume.callback = function() { ibox.gbOrders.changeCapacity(btnVolume, gtype, storage); }
...
game.ui.infobox.showWhSpecialOrdersWindow = function(parent, wh) {
  ...
  ibox.update = function() {
    ibox.btnToggleDevastation.text = wh.store().isDevastation()
                                    ? _u("stop_warehouse_devastation")
                                    : _u("devastate_warehouse");

    ibox.btnTradeCenter.text = wh.isTradeCenter()
                                  ? _u("become_warehouse")
                                  : _u("become_trade_center");
  }
