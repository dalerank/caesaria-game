...
int TTF_RenderUTF8_Blended_TX( unsigned int* data, int h, int p, TTF_Font *font, const char *text, SDL_Color fg)
{
    SDL_bool first;
    int xstart;
    int width, height;
    ...
    while ( textlen > 0 ) {
        ...
        for ( row = 0; row < glyph->pixmap.rows; ++row ) {
            ...
            if ( row+glyph->yoffset >= h ) {
                continue;
            }
            dst = (Uint32*)data +
                (row+glyph->yoffset) * p/4 +
                xstart + glyph->minx;
