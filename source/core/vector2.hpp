...
#ifndef __CAESARIA_VECTOR2_INCLUDED__
#define __CAESARIA_VECTOR2_INCLUDED__
...
public:
  ...
  Vector2<A> As() const
  {
	...
	float getAngleTrig() const
	{
		...
		if ( _y > 0)
			if (_x > 0)
																return atanf((float)_y/(float)_x) * math::RADTODEG;
			else
																return 180.0-atanf((float)_y/-(float)_x) * math::RADTODEG;
		else
			if (_x > 0)
																return 360.0-atanf(-(float)_y/(float)_x) * math::RADTODEG;
			else
																return 180.0+atanf(-(float)_y/-(float)_x) * math::RADTODEG;
	}
...
#endif //__CAESARIA_VECTOR2_INCLUDED__
