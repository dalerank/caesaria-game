...
bool Colosseum::build( const city::AreaInfo& info )
{
    ...
    size_t glSchools = info.city->statistic().objects.count(object::gladiatorSchool);
    size_t lionsNs = info.city->statistic().objects.count(object::lionsNursery);
...
std::string Colosseum::troubleDesc() const
{
    ...
    if (lions && gladiators)
    {
        return "##trouble_colosseum_full_work##";
    }
    else if (lions)
    {
        return "##colosseum_have_only_lions##";
    }
    else
    {
        return "##trouble_colosseum_have_only_gladiatros##";
    }
}
...
bool Colosseum::isShowGladiatorBattles() const
{
    return _getServiceManType() == Service::amphitheater;
}

bool Colosseum::isShowLionBattles() const
{
    return _getServiceManType() == Service::colosseum;
}

int Colosseum::maxVisitors() const
{
    return 1500;
}

DateTime Colosseum::lastAnimalBoutDate() const
{
    return _d->lastshow.lions;
}

DateTime Colosseum::lastGladiatorBoutDate() const
{
    return _d->lastshow.glads;
}
