...
container_linux::container_linux(gfx::Engine* painter)
{
  _painter = painter;
  m_temp_surface = Picture();
...
litehtml::uint_ptr container_linux::create_font( const litehtml::tchar_t* faceName, int size, int weight, litehtml::font_style italic, unsigned int decoration, litehtml::font_metrics* fm )
{
  ...
  Font fnt = Font::create( FONT_2 );

  bool found = false;
  found = true; //debug code
...
{
  cairo_font* fnt = (cairo_font*)hFont;
  gfx::Picture* cr		= (gfx::Picture*)hdc;
  ...
  fnt->font.draw( *cr, text, Point(x,y), true, false);
...
void container_linux::set_color(gfx::Engine* cr, litehtml::web_color color)	{ cairo_set_source_rgba(cr, color.red / 255.0, color.green / 255.0, color.blue / 255.0, color.alpha / 255.0); }
...
uint32_t* container_linux::surface_from_pixbuf(const gfx::Picture& bmp)
{
