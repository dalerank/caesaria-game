...
void Emperor::updateRelation(const std::string &cityname,
                             const RelationAbility &ability) {
  Relation &relation = _dfunc()->relations[cityname];
...
void Emperor::sendGift(const std::string &who, const std::string &type,
                       int value) {
...
void Emperor::Impl::resolveTroubleCities(const CityList &cities) {
  for (auto &city : cities) {
    ...
    float rule2destroy = utils::eventProbability(
        math::clamp(relation.wrathPoint / 100.f, 0.f, 1.f),
    ...
    unsigned int sldrNumber =
        std::max(legionSoldiersCount, relation.soldiers.sent);
