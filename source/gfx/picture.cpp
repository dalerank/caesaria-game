...
public:  
  ...
  Rect orect;
  SDL_Surface* surface;  // for SDL surface
...
void Picture::init( SDL_Surface* srf, unsigned int ogltx)
{
  _d->surface = srf;
  _d->opengltx = ogltx;
}
...
void Picture::setAlpha(unsigned char value)
{
  if( _d->surface )
  {
    SDL_SetAlpha( _d->surface, SDL_SRCALPHA, value );
  }
}
