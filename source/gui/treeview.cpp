...
{
    NColor textCol = NColor(0xffc0c0c0);

    if ( enabled() )
    {
        textCol = ( node == _selected ) ? ColorList::white : ColorList::black;
    }
...
void TreeView::draw( gfx::Engine& painter )
{
  ...
  if( _drawBack )
  {
    drawRect( absoluteRect(), ColorList::white, painter, 0 );
  }
  ...
  while( node )
  {
    ...
    if( node->hasChildren() )
    {
      ...
      drawRect( expanderRect, NColor(0xffc0c0c0), painter, clipRect );
      ...
      drawRect( Rect( center - offset, center + offset + Point( 1,1 ) ),
                ColorList::black, painter, clipRect );
    ...
    if( _linesVisible )
    {
      ...
      if( node->getParentItem() != _root )
      {
        ...
        drawRect( rc, NColor(0xffc0c0c0), painter, clipRect );
