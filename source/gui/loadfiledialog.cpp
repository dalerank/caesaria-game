  ...
  : Window( parent, rect, "", id ), _d( new Impl )
{
  ...
  CONNECT( _d->btnDelete, onClicked(), this,      LoadFileDialog::_fillFiles )
...
void LoadFileDialog::_fillFiles()
{
  if( !_d->lbxFiles )
    return;
  _d->lbxFiles->clear();
...
FileListBox* LoadFileDialog::_fileslbx() const { return _d->lbxFiles; }
const vfs::Directory& LoadFileDialog::_directory() const { return _d->directory; }
const std::string&LoadFileDialog::_extensions() const { return _d->fileExtensions; }
