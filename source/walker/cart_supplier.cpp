...
void CartSupplier::_reachedPathway()
{
  ...
  if( _pathwayRef().isReverse() )
  {
    ...
    GoodStore* storage = 0;
    FactoryPtr f = csDynamicCast<Factory>( building );
    GranaryPtr g = csDynamicCast<Granary>( building );
    WarehousePtr w = csDynamicCast<Warehouse>( building );
                                 ...
                                 Pathway &oPathWay, long& reservId )
{
       ...
       pathWayIt != pathWayList.end(); ++pathWayIt)
  {
    ...
    BuildingPtr building= csDynamicCast<Building>( pathWayIt->first );
    Pathway& pathWay= pathWayIt->second;

    SmartPtr< T > destBuilding = csDynamicCast<T>( building );
...
void CartSupplier::computeWalkerDestination(BuildingPtr building, const Good::Type type, const int qty )
{
  ...
  pathPropagator.init( csDynamicCast<Construction>( building ) );
...
void CartSupplier::_reserveStorage()
{
  ...
  GoodStore* storage = 0;
  FactoryPtr f = csDynamicCast<Factory>( b );
  GranaryPtr g = csDynamicCast<Granary>( b );
  WarehousePtr w = csDynamicCast<Warehouse>( b );
  if( f.isValid() ) { storage = &f->getGoodStore(); }
  else if( g.isValid() ) { storage = &g->getGoodStore(); }
  else if( w.isValid() ){ storage = &w->getGoodStore(); }
