...
void Indigene::_updateState()
{
  ...
  case Impl::findField:
  {
    ...
    NativeFieldList fields = helper.find<NativeField>( building::nativeField, pos() - offset, pos() + offset );
    foreach( i, fields )
    {
  ...
  case Impl::go2center:
  {
    TilePos offset( 1, 1 );
    city::Helper helper( _city() );
    NativeCenterList centerList = helper.find<NativeCenter>( building::nativeCenter, pos() - offset, pos() + offset );
    if( !centerList.empty() )
    {
  ...
  case Impl::back2base:
  {
    ...
    TilePos offset( 1, 1 );
    BuildingList huts = helper.find<Building>( building::nativeHut, pos() - offset, pos() + offset );
